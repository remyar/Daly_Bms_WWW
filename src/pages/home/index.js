...
function Home(props) {
    ...
    const controllers = props.globalState.controllers || [];
    ...
    return <div>
        {(() => {
            ...
            for (let i = 0; i < controllers.length; i++) {
                ...
                infosTab.push(<div id={"range" + i} className="stat"><span className="x t">Range:</span><span className="x v">{controller.cellDiff.toFixed(2) + " mV"}</span></div>);
                infosTab.push(<div id={"voltage" + i} className="stat"><span className="x t">Voltage:</span><span className="x v">{controller.packVoltage.toFixed(2) + " V"}</span></div>);
               
                if (controller.bmsHeartBeat && controller.bmsHeartBeat > 0) {
                    infosTab.push(<div id="sent" className="stat"><span className="x t">HeartBeat:</span><span className="x v">{controller.bmsHeartBeat}</span></div>);
                }
